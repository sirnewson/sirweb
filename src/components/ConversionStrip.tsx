    ...
    <section className="relative z-10 px-6 py-16 md:py-20 border-y border-white/5 bg-neutral-dark/70 backdrop-blur">
      <div className="max-w-7xl mx-auto">
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-end mb-12">
          <div>
            <p className="text-primary text-xs font-black uppercase tracking-[0.25em] mb-4">Work with Sir Newson</p>
          ...
          <p className="text-white/60 text-base md:text-lg leading-relaxed max-w-2xl lg:ml-auto">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {offers.map((offer, index) => (
            <motion.div
              ...
              className="group rounded-3xl border border-white/10 bg-white/[0.03] p-7 hover:border-primary/50 hover:bg-white/[0.06] transition-all duration-300"
            >
              <div className="mb-8 flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/10 text-primary font-black group-hover:bg-primary group-hover:text-black transition-colors">
                0{index + 1}
              </div>
              <h3 className="font-display text-2xl font-black text-white mb-4">{offer.title}</h3>
              <p className="text-white/55 text-sm leading-7 mb-7">{offer.copy}</p>
              <Link to={offer.path} className="inline-flex items-center gap-3 text-primary font-black text-sm uppercase tracking-wider group-hover:gap-5 transition-all">
                {offer.cta}
