        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {services.map((service, index) => (
            <ScrollReveal
              ...
              duration={0.6}
            >
              <article className="group rounded-[2rem] border border-white/10 bg-white/[0.03] p-8 hover:border-primary/50 hover:bg-white/[0.06] transition-all duration-300 h-full flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between mb-10">
                    <Float y={2.5} duration={4 + index} className="w-fit">
                      <span className="text-primary font-black tracking-widest block">{service.label}</span>
                    </Float>
